...
** Last update Mon Mar 21 17:01:02 2016 bougon_p
*/

#include "asm.h"

int	or_arg_1(int fd, t_info *info, char byte)
{
  char	check;
...
int	or_arg_2(int fd, t_info *info, char byte)
{
  char	check;
...
int	w_or(t_info *info, int fd)
{
  ...
  if ((byte = w_coding_byte(fd, info)) == -1)
    return (1);
  if (or_arg_1(fd, info, byte) == 1)
    return (1);
  if (or_arg_2(fd, info, byte) == 1)
    return (1);
