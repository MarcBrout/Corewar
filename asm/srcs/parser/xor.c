...
** Last update Mon Mar 21 17:16:33 2016 bougon_p
*/

#include "asm.h"

int	xor_arg_1(int fd, t_info *info, char byte)
{
  char	check;
...
int	xor_arg_2(int fd, t_info *info, char byte)
{
  char	check;
...
int	w_xor(t_info *info, int fd)
{
  ...
  if ((byte = w_coding_byte(fd, info)) == -1)
    return (1);
  if (xor_arg_1(fd, info, byte) == 1)
    return (1);
  if (xor_arg_2(fd, info, byte) == 1)
    return (1);
