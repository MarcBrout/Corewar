...
** Last update Fri Mar 18 18:46:56 2016 bougon_p
*/

#include "asm.h"

int	and_arg_1(int fd, t_info *info, char byte)
{
  char  check;
...
int	and_arg_2(int fd, t_info *info, char byte)
{
  char  check;
...
int	w_and(t_info *info, int fd)
{
  ...
  if ((byte = w_coding_byte(fd, info)) == -1)
    return (1);
  if (and_arg_1(fd, info, byte) == 1)
    return (1);
  if (and_arg_2(fd, info, byte) == 1)
    return (1);
